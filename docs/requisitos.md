...
* **U (Atualizar):** Permitir a correção de medidas ou informações registradas incorretamente.
* **D (Excluir):** Permitir a exclusão controlada de registros duplicados ou lançados por engano.
...
**Justificativa:** O CRUD é indispensável para que o NutriMirim mantenha a persistência local (*offline-first*) dos dados e sustente o acompanhamento longitudinal do crescimento infantil. Operações sensíveis, como exclusões e edições, devem possuir controle de acesso para evitar alterações indevidas e perda acidental do histórico de avaliações.
